...
class VisionExtractor(BaseExtractionStrategy):
    ...
    def _estimate_cost(self, total_tokens: int) -> float:
        return float(total_tokens) * 0.000002
        ...
        with pdfplumber.open(file_path) as pdf:
            for page_number, page in enumerate(pdf.pages[: self.max_pages], start=1):
                ...
                if baseline_text.strip():
                    ...
                    page_confidence = 0.65

                if tables:
                    page_confidence = max(page_confidence, 0.75)
                ...
                if self.api_key and not self._budget_exhausted:
                    ...
                    if response_data:
                        ...
                        page_confidence = max(page_confidence, 0.90 if text_blocks else 0.55)

                if self._budget_exhausted:
                    page_confidence = min(page_confidence, 0.85)
